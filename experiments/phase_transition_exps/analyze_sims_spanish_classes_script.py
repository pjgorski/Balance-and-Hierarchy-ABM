...
for school in datanames:
    ...
    df_rho, df_files, last_file_processed = process_folder(outputs_dir, 
                                  no_triad_stats=False, max_num_rows=1000000, dataset = school, external_dataset_stats_file = "results_realnet_spanish_class.h5")
    path = Path(fpath)
...
out_files = [outputs_dir.joinpath("df2_rho_" + school + ".h5") for school in datanames]
