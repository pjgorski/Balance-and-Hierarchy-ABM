# This file was used to create simulations of ABM on Epinions dataset. 
...
dataset = "Epinions"
wiki_fname = "../data/epinions/epinions_triads.csv"
...
what = "rho_inits22longer"
# what = "rho_inits5"
# what = "test_run2"

if what == "rho_inits22":
    foldername = "outputs/epinions-rhoinits-triads"
    
    sim_sets = [[[0.85, 0.875, 0.925, 0.95], [0.8, 0.9, 1.0], [0, 0.1, 0.2]], 
                 ]
...
elif what == "rho_inits22longer":
    foldername = "outputs/epinions-rhoinits-triads-replonger"
    
    sim_sets = [[[0.925, 0.95], [0.8, 0.9, 1.0], [0, 0.1, 0.2], [0.4, 0.6, 0.8, 0.9]], 
                 ]
    ...
    sim_sets = [[list(np.arange(0.2, 0.95, 0.2)), list(np.arange(0, 1.01, 0.1)), list(np.arange(0, 1.01, 0.1))], 
    ...
    sim_sets = [[list(np.arange(0.1, 0.95, 0.2)), list(np.arange(0.1, 1.01, 0.2)), list(np.arange(0.1, 1.01, 0.2))], 
    ...
    control_initial_rho = True
    keep_rho_at = [0.77, 0.97, 0.75, 0.98]
elif what == "rho_inits32":
    foldername = "outputs/epinions-rhoinits-triads"
    
    sim_sets = [[0.1, 0.9000000000000001, 0.30000000000000004],
    ...
    control_initial_rho = True
    keep_rho_at = [0.77, 0.97, 0.75, 0.98]
elif what == "rho_inits4":
    foldername = "outputs/epinions-rhoinits-triads"
    
    sim_sets = [[list(np.arange(0.1, 0.95, 0.2)), list(np.arange(0., 1.01, 0.2)), list(np.arange(0.1, 1.01, 0.2))], 
                # [0.0, list(np.arange(0,1.01, 0.1)), 0.5], [1.0, 0.5, list(np.arange(0,1.01, 0.1))]
                 ]
    ...
    control_initial_rho = True
    keep_rho_at = [0.77, 0.97, 0.75, 0.98]
elif what == "rho_inits5":
    foldername = "outputs/epinions-rhoinits-triads"
    
    sim_sets = [[list(np.arange(0.1, 0.95, 0.2)), list(np.arange(0.1, 1.01, 0.2)), list(np.arange(0., 1.01, 0.2))], 
                # [0.0, list(np.arange(0,1.01, 0.1)), 0.5], [1.0, 0.5, list(np.arange(0,1.01, 0.1))]
                 ]
...
elif what in ["rho_inits2", "rho_inits22", "rho_inits3", "rho_inits32", "rho_inits4", "rho_inits5", "test_run", "test_run2"]:
    exp_sets = explode_sims(sim_sets)
...
elif what in ["rho_inits22longer", ]:
    exp_sets = explode_sims(sim_sets)
    
    # rho_init = rho_inits
    
    all_args = [prepare_args2(dataset, network, single_sim_set, single_sim_set[3], 
                              steps, saving_multiplier, reps, False, 
                              foldername, keep_rho_at, control_initial_rho) for single_sim_set in exp_sets]
